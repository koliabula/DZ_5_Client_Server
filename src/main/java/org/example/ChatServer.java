    ...
    public void run() {
      ...
      while (true) {
        ...
        if (SendMessageRequest.TYPE.equals(type)) {
          ...
          final BroadcastMessageRequest bRequest;
          try {
            bRequest = objectMapper.reader().readValue(msgFromClient, BroadcastMessageRequest.class);
          } catch (IOException e) {
            System.err.println("Не удалось прочитать сообщение от клиента [" + clientLogin + "]: " + e.getMessage());
            sendMessage("Не удалось прочитать сообщение SendMessageRequest: " + e.getMessage());
            continue;
          ...
          ClientHandler clientTo = clients.get(bRequest.getRecipient());
          for (ClientHandler ch: clients.values()) {
              if (!ch.equals(clientTo)) {
                ch.sendMessage(bRequest.getRecipient() + ": " + bRequest.getMessage());
              }
          }


        } else if (DisconnectRequest.TYPE.equals(type)) { // DisconnectRequest.TYPE.equals(type)
          final DisconnectRequest dRequest;
          try {
            dRequest = objectMapper.reader().readValue(msgFromClient, DisconnectRequest.class);
          } catch (IOException e) {
            System.err.println("Не удалось прочитать сообщение от клиента [" + clientLogin + "]: " + e.getMessage());
            sendMessage("Не удалось прочитать сообщение SendMessageRequest: " + e.getMessage());
            continue;
          }
          ClientHandler clientTo = clients.get(dRequest.getRecipient());
          for (ClientHandler ch : clients.values()) {
            if (!ch.equals(clientTo)) {
              ch.sendMessage(dRequest.getRecipient() + ": " + dRequest.getMessage());
            }
          }
